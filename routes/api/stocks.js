...
router.get("/batch/:symbols", (req, res) => {
    axios.get(`https://${process.env.NODE_ENV === "production" ? "cloud" : "sandbox"}.iexapis.com/stable/stock/market/batch?symbols=${req.params.symbols}&filter=symbol,companyName,latestPrice,latestUpdate&types=quote&token=${iexApiToken}`)
...
router.get("/lookup/:symbol", (req, res) => {
    axios.get(`https://${process.env.NODE_ENV === "production" ? "cloud" : "sandbox"}.iexapis.com/stable/stock/${req.params.symbol}/quote?filter=symbol,companyName,latestPrice,latestUpdate&token=${iexApiToken}`)
...
router.get("/chart/:symbol/:range", (req, res) => {
    ...
    axios.get(`https://${process.env.NODE_ENV === "production" ? "cloud" : "sandbox"}.iexapis.com/stable/stock/${req.params.symbol}/chart/${rangeSubUrl[req.params.range]}&token=${req.params.range === "1d" ? iexApiToken : iexSandboxToken}`)
